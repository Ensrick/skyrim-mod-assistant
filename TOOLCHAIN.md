...
| NIF inspection and LE-to-SE conversion | local `nif-port-cli` + current nifly | `Ensrick/nif-port-cli@46c6deb` (PR #2 merged; upstream `ousnius/SSE-NIF-Optimizer@dbba8b3`, `nifly@846518b`) | Source-built, fully headless, fail-closed conversion with post-save reload validation; adds explicit `convert-sse --headparts` with all-dynamic-geometry assertion; 52/52 DDS Workshop 0.3 base NIFs preserved shape names, geometry, skinning, bones, and texture paths in an audit conversion |
