...
- **What (tree):** 0.5.0: `Rules/Staff` (a held staff is pinned to `hand.left` / `hand.right` and shown there with Walking Stick 1.1.1's offsets under `NPC L Hand [LHnd]` / `NPC R Hand [RHnd]`, culled while drawn; a staff that reaches the inventory takes a free hand, off-hand first, one frame later; a staff that leaves a hand, including by a two-handed weapon or bow taking both hands, is deposited one frame later with one HUD line; the pickup gate accepts a staff only into a free hand); the gates (`Rules/GateRules` pure + CTest, `Rules/Gates`, a `TESFurnitureEvent` sink, `Display/GatePoll`: a 250 ms thread that queues one main-thread task at a time while an actor has a hand display, never re-queued from a task, stopped at DLL detach); placement hints (`Rules/PlacementHints`, H9) for gear nodes 11 and 12; the two conditions Walking Stick's JSON names, `IED_GearNodePlacementHint` and `IED_GearNodeEquippedPlacementHint`, registered at `kPostLoad` through Open Animation Replacer's public conditions API (Ersh's three API headers copied verbatim into `extern/`, GPL-3.0 with the Modding Exception, the `.cpp` half rewritten over `REX::W32` so no new import); `tools/oar_requiredplugin_rewrite.py` rewrites a pack's `requiredPlugin` AND `requiredVersion` (OAR checks both; the spec counted one string); the other four `IED_*` names deliberately not registered (component names documented nowhere this session may read); the stash `EnsrickEquipmentDisplay.esl` written as Spriggit YAML by hand from the Mutagen field names of `PlayerHouseChest` and `WIDeadBodyCleanupCell` and converted with Spriggit 0.41.0 (`tools/build_esl.ps1`: two byte-identical conversions, exact round trip, `skyrim-record-cli` 3 records / 2 links / 0 unresolved; ESL-flagged, master `Skyrim.esm`, one CONT + one CELL + one persistent REFR `0x802`, nothing vanilla touched, sha256 `885eb320...`, 872 bytes); `[Storage]` defaults to it so the reconcile's overflow lands in the chest (D2 final; the drop at the feet is now only the fallback when the plugin is not enabled); the self-test grown to 25 steps; `/d1trimfile` on every target including CommonLibSSE plus CTest `no_checkout_path`, after finding that 0.4.3 built different bytes in the r2 and r3 worktrees from the same commit (21 absolute paths through `std::source_location` in the library's own files). 0.5.1: the reconcile plans with and without the arriving copies and bounces an arrival by the difference (launch 1 showed an Iron War Axe taken back from a barrel displacing the Iron Mace by form-id order and leaving it refused); the self-test waits `SelfTestSettleSeconds` (20) of live play before step 1 so Unbound's own gear has landed. Every version: three builds byte-identical (0.5.0 `d29d1850...`, 0.5.1 `92b12277...`), CTest 9/9, version gate PASS, no USER32 import, no checkout path.
- **What (profile):** under claim `fable/eed-r3`: `Ensrick - Equipment Display` re-staged with 0.5.0 (`20260911T225238018Z-03dcda355712`) and, new, `EnsrickEquipmentDisplay.esl` enabled (`20260911T225238459Z-67ec09166ff9`; 304 active; `verify_order` CLEAN; LOOT run report-only against a copy of `plugins.txt`: the ESL is a light master with no messages; the VFS run's own profile re-serialization moved it from the end into the master block, every other plugin's order checked identical to the R2 close-out manifest), weapon-balance and full-cloak proofs regenerated for the 304 set (`WeaponBalancePatch.esp` byte-identical, the ESL adds no WEAP; install `20260911T230011051Z-b95bf578b927`, cloak `20260911T230108620Z-6687627129ec`), then 0.5.1 (`20260911T231945315Z-4436737e96fd`), three launches, then plugin-disable `20260911T234023325Z-c6c71a31322f` and mod-disable `20260911T234023390Z-a8712fb22b7a` (303 active), the two proofs regenerated again for the exit state (weapon `20260911T234418198Z-e6c6a62ee394`, package byte-identical to R2's close-out; cloak `20260911T234514139Z-1eec8c3b1c9e`), preflight = the two expected no-Keep rows only, ledger row updated in the working tree, claim released. Walking Stick 1.1.1 (Nexus 120966 file 508070, zip sha256 `90bf8aa9...`) downloaded to `records-work/walking-stick-120966/` for reference and converted (5 conditions in 3 `config.json` files), **not installed** (#270) and not committed.
- **Verification:** launch 1 (0.5.0, `records/log-snapshots/20260911-180247-eed-r3-1/`): **PASS on the launch criteria** (main menu 45.2 s, 180 s soak, Journal quit, exit 0, no crash, 0 `[error]`), self-test 41/49 with every FAIL on the self-test's side or the gap above (Unbound's gear landing mid-scenario; the displaced mace; a skip that also skipped the unequip step). Launches 2 and 3 (0.5.1, `20260911-182320-eed-r3-2/`, `20260911-183138-eed-r3-3/`): **PASS**, main menu 36.5 / 38.2 s, fully automated Unbound walk (Sword + Dual + Staff of Flames, start = Sleeping Giant Inn), self-test 55/55 each, soaks, Journal quits, exit 0, no crash, 0 `[error]`, PopupGuard nothing. Log lines per rule (all three launches unless noted): OAR's own log `RequestPluginAPI_Conditions called ... EnsrickEquipmentDisplay` and ours `condition registered: IED_GearNodePlacementHint / IED_GearNodeEquippedPlacementHint under 'EnsrickEquipmentDisplay' 0.5.x.0: OK`; `storage: stash FE054802 'Long-term Storage' ... persistent=true, cell FE054801`; a staff with both hands taken and Unbound's own Staff of Flames with both hands taken -> `stored in Long-term Storage`; Unbound's real `AddItem` of the Staff of Flames with a hand free (launches 2-3) -> `arrived ... with the left hand free: equipping it there (rule 4, off-hand first)`; `hint: actor 00000014 gear node 12 = 16 (hand.left shown)` -> `attached hand.left ... (staff) to 'NPC L Hand [LHnd]'`; the four in-process evaluations `IED_GearNodePlacementHint(11==16)` false, `(12==16)` true, `IED_GearNodeEquippedPlacementHint(Left hand false==16)` false, `(true==16)` true; `culled while drawn` / `visible again`; the furniture gate on an inn chair (launches 2-3) `gate poll: gates now [sitting furniture]; refreshing` -> `hidden by gate !sitting`, hint 0, then `visible again`, hint 16; unequip -> `left the hands of actor 00000014: stored in Long-term Storage (rule 4)` with the HUD line and the copy detached; D3 (Iron Greatsword equipped over the held staff) -> the same deposit, stash count 3; the Iron Mace with the hips full -> `stored in Long-term Storage` (D2 final); the displacement fix (launches 2-3) `took a slot from a copy already carried ... the arrival leaves -> returned to Chest`; `nothing was ever dropped at the feet` every time. 0 crashes in 3. One self-healed warning in launch 3 (`copy missing from 'WeaponMaceLeft', rebuilding`). **UNVERIFIED:** swimming and riding (no scripted way in; the same poll path as the furniture gate); Walking Stick's animations themselves (the mod is not installed); the copies' look on the body and the hand offsets (headless); NPCs; quest items; a human picking a staff up from the world; the InventoryMenu dump. Not a save-load PASS in the `launch_verify` sense (fresh character by design, #262/#268). Receipt `records/source-builds/ensrick-equipment-display-r3.json`. Mod left DISABLED.

## 2026-09-11 17:15 - EnsrickEquipmentDisplay R2 (limit = slots) built, launched four times, one hang fixed; verified as far as the receipt says, left DISABLED

- **Source:** user, 2026-09-10: *"Well, finish it."* #269 (phases R1-R4 of `docs/VISIBLE-EQUIPMENT-PLUGIN-DESIGN-2026-09-10.md`), #36 (the rules), decisions D1-D5 in #269. Agent Fable/eed-r2. Tree `skyrim-tools-source/EnsrickEquipmentDisplay` (`github.com/Ensrick/EnsrickEquipmentDisplay`, private): main fast-forwarded to `d9cd9b0` = `732e72a` 0.3.2 (eed-r1) + `b051084` 0.3.3 + `6f98590` 0.4.0 + `d4208d9` 0.4.1 + `bf75d6f` 0.4.2 + `d9cd9b0` 0.4.3, all built in the `EnsrickEquipmentDisplay-r2` worktree on branch `r2`.
- **What (tree):** 0.3.3: the R1 launched-vs-clean DLL difference (`141576f0` vs `fc7fc730`) is explained and reproduced: an incremental link reuses the build directory's PDB and bumps its age 1 to 2, the age sits in the CodeView record, and `/Brepro` derives the timestamp, GUID and Repro hash from the image, so 73 bytes change while `.text` is identical; fix = PRE_LINK removal of the PDB, `/PDBALTPATH:%_PDB%` (bytes no longer depend on the checkout path), CTest `reproducible_link`; also bumps the version record 0.3.2 forgot. 0.4.0 (R2 per the spec): `PlayerCharacter::PickUpObject` (vfunc 0xCC) refuses a pickup with no free slot and leaves the reference in the world with one HUD line; `AddObjectToContainer` (0x5A) logs and defers because the engine calls it from inside the source's `RemoveItem` (an inline refusal would be a silent delete); a one-frame-later reconcile returns copies with no slot to their source container or actor, else `Storage::Deposit` (the `[Storage] StashPlugin/StashFormID` reference, else the engine's own drop at the feet; never a delete); two packed daggers with the D4 `PackedDaggersNeedBackpack`/`BackpackKeyword` key; quest items not exempt (no quest parameter anywhere); SKSE action events for the drawn state; the board line reads `inventory N = visible V + packed P + refused R`; a `[Diagnostics] SelfTest` scenario (off by default) for headless verification. 0.4.1: the self-test's SKSE task re-queued itself and SKSE drains its queue until empty each frame (launch 1 hang); a timer thread now adds the step once. 0.4.2: the self-test waits while `UI::GameIsPaused()`. 0.4.3: the self-test first moves the actor's pre-existing weapons out so the board starts empty. Every version: two clean builds byte-identical (0.4.x also equal to their incremental builds), CTest 5/5, version gate PASS, no USER32 import. NOT in this build: the plugin-shipped ESL stash (D1, Creation Kit work), hence route-to-storage ran as the drop fallback.
- **What (profile):** `Ensrick - Equipment Display` re-enabled under claim `fable/eed-r2` with each launched clean DLL and a verification ini (`Rules Enabled=1`, `SelfTest=1`, `DumpPlacements=1`, `Level=debug`), launched four times on the private desktop (`records-work/eed-r2-run.ps1`, which refuses to launch unless sha256(installed DLL) == the receipt's clean-build hash and `verify_order` is CLEAN), then disabled again (`20260911T220143286Z-0a26f13a8778`); the folder keeps the 0.4.3 DLL `2b0ad9df...` and the verification ini. No plugin added; verify_order CLEAN before and after; no cloak refresh (DLL-only, reverted). `audit/launch_skyrim_isolated.ps1` now accepts `-WaitSeconds` up to 1800 (the 600/900 cap cut two quits). The ledger row is updated in the working tree only (the file carries another session's hunks).
- **Verification:** launch 1 (0.4.0 `38c467f1`): **FAILED**, attributed: main-thread hang at `kNewGame`, no crash, spindump shows this DLL's frames in SKSE's task queue; controller budget exit 75 (`records/log-snapshots/20260911-154746-eed-r2-1/`). Launch 1b (0.4.1 `f5c8f2c0`): rules evidence 17/18 in the Unbound start room under an open menu (the draw check cannot play under a menu), cull-when-drawn proven at world entry, Unbound's own Steel Mace `AddItem` dropped by the D2 route; no crash in 15 min; the 900 s budget cut the quit (`20260911-161254-eed-r2-1b/`). Launch 2 (0.4.2 `15d555ff`): **PASS** on the launch criteria (main menu 32.4 s, world entry, 180 s soak, Journal quit, exit 0, 0 `[error]`, no crash); 7/18 checks skewed by Unbound's two pre-existing maces, every one a correct refusal (`20260911-163659-eed-r2-2/`). Launch 3 (0.4.3 `2b0ad9df`): **PASS**, self-test 18/18 on a live actor: board `inventory 6 = visible 4 + packed 2`; left-hand axe on `WeaponAxeLeft` `culled while drawn` then `visible again`; two daggers packed hidden; `pickup refused ... reference stays in the world` with the HUD line and inventory unchanged; scripted mace with the hips full `dropped at your feet` (D2, no stash); container transfer `returned to Sack`; accepted pickup once a slot freed; 180 s soak, Journal quit, exit 0, 0 `[error]`, no crash, PopupGuard nothing (`20260911-165411-eed-r2-3/`). Main menu 36.5 / 33.3 / 32.4 / 39.8 s, all under 60 s. 0 crashes in 4 (#273 not seen). **UNVERIFIED:** the copies' look on the body (headless); a human activating a world weapon (the refusal was driven by the plugin's self-test through the real hook); quest items (none appeared); the InventoryMenu dump (the Tween menu never opened from MenuPilot); route-to-storage into a stash (none configured); NPCs. Not a save-load PASS in the `launch_verify` sense (fresh character by design, #262/#268). Receipt `records/source-builds/ensrick-equipment-display-r2.json`.
- **Close-out (17:08-17:20, claim re-acquired):** the lead's baseline messages (BTPS removed by the user at 15:26, active plugins 304 -> 303, weapon-balance and full-cloak gates re-invalidated) reached this agent only after launch 3, so the four launches ran with those two freshness gates RED (the isolated launcher runs no preflight; the gates concern weapon speed and cloak slots, not the plugin under test) and on 303 plugins where R0/R1 ran on 304. Then: `Immersive Equipment Displays` (disabled, 62001, SlavicPotato exclusion rule 1) trashed recoverably (`mod-trash` `20260911T221021692Z-dbb5b7affd41`, `.mo2-headless-trash/`; no overlay folder existed; the ledger never had a 62001 row); Weapon Balance regenerated for the 303 set (382 inputs, `WeaponBalancePatch.esp` `52a5e2c4...`, 3504 WEAP overrides, deterministic double build, final-winner audit PASS, package `d90b7ac9...`, `mod-install --replace` `20260911T221507715Z-b93f5e23b935`, freshness gate `filesWritten 0`); Full Cloak Exclusivity rebuilt for the final fingerprint (240 directives, 28680 exclusions, `mod-stage --replace` `20260911T221604733Z-e729c189d500`, `check_installed` clean); preflight now shows only the two expected no-Keep rows (34705, 50049). D2's drop-at-the-feet is recorded as INTERIM (R3 swaps the stash into the same `Storage::Deposit`). Tree 0.4.4 (`ed0482e`, branch `r2`): the self-test timer thread stops at DLL detach, `docs/DESIGN.md` states the no-requeue rule; three builds `86cb6691...`, CTest 5/5, **not launched**; `main` fast-forwarded to it (`ed0482e`) on the lead's instruction, the launched build stays 0.4.3.
...
- **What (tree):** 0.2.1: CommonLibSSE-NG from the Ensrick no-modal branch (`68b6ba48d`, `COMMONLIBSSE_NO_MODAL_ERRORS`), so the DLL imports no USER32 at all; `/W4 /WX /Brepro`; CTest (rule engine lifted from the retired scaffold, a PE import gate, a source grep gate); the SKSE version record written by hand with the Address Library format-5 bit; CI. 0.3.0: R1 per the spec (H1-H4, H8, H11): one `DisplayRequest` applier, the placement table (defaults + `placements.json` + user overlay, CTest), `Rules::Policy` rebuilding the slot board from the inventory with one log line per decision, a container-changed refresh sink, `DumpPlacements`, `SkeletonCheck`; all display work marshalled to the main thread through the SKSE task interface after R0. 0.3.1: `NiObject::Clone()` instead of `CreateDeepCopy`, collision stripped from copies. `[Rules] Enabled=0` in the shipped ini until R2.
- **What (profile):** `Ensrick - Equipment Display` re-enabled under claim `fable/eed-r1` with the R1 DLL and a verification ini (`Rules Enabled=1`, `DumpPlacements=1`, `Level=debug`), launched twice on the private desktop, then disabled again (`20260911T041816047Z-ef5e107efa22`), as R0 left it. The folder keeps the 0.3.1 DLL and the verification ini. No plugin added; no cloak refresh (DLL-only mod, reverted; R0 had shown the directives byte-identical with it enabled).
- **Verification:** attempt 1 (0.3.0, `413c4fd2...`): main menu 41.0 s, loaded, the rules engine placed the first Unbound weapon, then **FAILED**: crash 5 s after world entry inside the engine's `NiObject::CreateDeepCopy` called from our `Attach::BuildDisplay` (attributed; `records/log-snapshots/20260910-230151-eed-r1-attempt1/`). Attempt 2 (0.3.1, `141576f0...`): **PASS for the R1 criteria**: main menu 46.8 s, fresh Unbound character, both `Load3D` hooks and both sinks live, the player tree on the main-thread task had all 8 roster nodes (257 nodes, `XPMSE 3.60`; R0's 167-node view was the loader thread's), the rules engine decided on four real inventory events (Hunting Bow -> `back.bow` attached on `WeaponBow`; Iron Mace -> `hip.right.1h` attached on `WeaponMace`; mace equipped -> engine-drawn, copy detached; Unbound removed both -> board empty, copies detached; the InventoryMenu dump confirmed no weapon carried), 180 s soak, 0 `[error]`, no crash log, PopupGuard intercepted nothing, Journal quit to desktop, controller exit 0 (`records/log-snapshots/20260910-231046-eed-r1/`). **Stricter protocol (team lead, 23:31-23:56):** three more fresh-character launches with the clean 0.3.1 DLL (`fc7fc730…`), Unbound's Equipment MCM driven to Sword + Dual + Staff of Flames + Solitude each time: **0 crashes in 3**; every launch attached both sword copies on `WeaponSword` and `WeaponSwordLeft`, the staff went to the right hand (hand slots are R3), draw/sheathe with copies attached was clean, 180 s soaks, inventory dumps matched the board (2 swords + staff = visible 3), Journal quits, controller exit 0. Main menu 49.6 / **65.1** / 34.5 s (B over the 60 s criterion; not attributed). Ledger row added (`records/installed-mods.json`, atomic replace, then `enabled=false`), weapon gate PASS before and after (no #253 drift), `audit/launch_skyrim_isolated.ps1` `-WaitSeconds` cap raised 600 to 900. Snapshots `records/log-snapshots/20260910-23{3224,4112,4858}-eed-r1-{A,B,C}/`. **UNVERIFIED:** the copies' look on the body (headless), a weapon equipped in the left hand (cull-when-drawn path), packed daggers, a refused copy, NPCs. Not a save-load PASS in the `launch_verify` sense (fresh character by design, #262/#268). Receipt `records/source-builds/ensrick-equipment-display-r1.json`.

## 2026-09-10 22:35 — EnsrickEquipmentDisplay phase 2, R0 verification launch: control PASS, plugin 1 crash in 2, left DISABLED

- **What:** First in-game run of the first-party `EnsrickEquipmentDisplay.dll` (phase 2 dual-sheath build, sha256 `b03a776e24feed796805ee62fdced7f054706d3e516f44ce285081b669d1e205`, source `skyrim-tools-source/EnsrickEquipmentDisplay` at `662d82f`, not edited). Staged as MO2 mod `Ensrick - Equipment Display` (`mod-stage` `20260911T030741243Z-3d47153d3580`, priority 299 next to Popup Guard 298 / Window Focus Guard 297, no plugin added) with the shipped ini plus `[Diagnostics] DumpSkeleton=1`. Before that, a #253 recurrence (Default's order drift from the user's 14:30 GUI session) was restored (`Thanedom Assets.esl` / `SmoothCam.esl`, transactions `20260911T024541650Z-bb4a361f6e3f`, `20260911T024631041Z-87e607981466`; backups `*.bak.v20260910-214600-pre-eed-r0-order-restore`) so the weapon gate passed with no regeneration and the cloak proof returned to Astra's 09:39 fingerprint; the cloak reservation was rebuilt for the new enabled-mod fingerprint (`20260911T031310523Z-bec93b6420f1`, 240 directives byte-identical, 569/576 meshes, 0 parse errors, 0 slot-58 hits) after a second order restore (`20260911T031137898Z-d1d586bfc4ad`) undid the swap that the first control run's exit had written into Default (`plugins.txt` 21:52:42). **Reverted at the end:** `mod-disable` `20260911T033131024Z-9dd44697e050` (folder kept, now holding the attempt-2 debug ini), cloak package restored byte-for-byte from `records-work/ussep439c-cloak-20260910/package/mod` (`20260911T033131075Z-568bb6ef77ac`), `check_installed` clean, order on Astra's baseline. Simple Dual Sheath untouched (enabled). No ledger row added (the mod ends disabled; the ledger file is dirty from another session).
- **Source:** User 09-10 *"Revive IED, make our own version"*; [#269](https://github.com/Ensrick/skyrim-mod-assistant/issues/269) phases 0-2 by the owning session; R0 dispatched by the team lead. Receipt [records/source-builds/ensrick-equipment-display-phase2-r0.json](records/source-builds/ensrick-equipment-display-phase2-r0.json); logs `records/log-snapshots/20260910-{214825,215743}-eed-r0-control/`, `20260910-221408-eed-r0-plugin/`, `20260910-221942-eed-r0-plugin2/`.
- **Verification:** **CONTROL PASS / PLUGIN FAIL (1 crash in 2 attempts) / phase 2 UNVERIFIED.** All game runs headless: control attempt 1 on the interactive desktop (`launch_verify --no-autoload`, main menu 47.6 s) was interrupted by the user at the controls (#164: unpiloted Journal opens, F5 quicksave, quit 21:52:40; no crash) and its two saves (Save1_09B88E99 and Quicksave0_09B88E99 with their .skse co-saves) were afterwards moved, not deleted, from `Documents\My Games\Skyrim Special Edition\Saves` to `records-work/eed-r0-throwaway-saves/` on the lead's instruction so Continue points at the user's own Save3_43E399CB again; every later run used `launch_skyrim_isolated.ps1` (private desktop, muted Default clone, private saves). Control: main menu 39.7 s, MenuPilot New Game -> character -> Unbound MCM City > Solitude -> Begin, world entry 22:00:29, 355 s unpaused, no crash, normal quit, controller exit 0. Plugin attempt 1 (shipped ini): main menu 39.9 s, `skse64.log` "EnsrickEquipmentDisplay.dll ... loaded correctly (handle 16)", 0 refused, PopupGuard intercepted nothing, then **crash 6 s after Begin** (`crash-2026-09-10-22-16-00.log`: AV at `SkyrimSE.exe+0CD19BD`, null vtable call on a `BSAudioManagerThread` worker, EED loaded but not on the stack; a signature seen in none of the 20 earlier crash logs); the plugin log was 0 bytes because the shipped `FlushLevel=warn` never flushed. Plugin attempt 2 (same DLL, `Level=debug FlushLevel=debug`): loads, `hooked Character::Load3D`, `hooked PlayerCharacter::Load3D`, `registered equip event sink`, `wrote skeleton dump`, world entry 22:25:18, **181 s no crash**, quit confirmed through the game's dialog 22:28:38 but the controller's 600 s budget expired before observing the exit (returned 75). No `attached` line and no warning in either attempt: no left-hand weapon or staff was ever equipped, so the dual-sheath path never ran. The player skeleton dump (167 nodes, identical both times) contains **none** of the XPMSSE slot nodes the SlotTable targets (no `Weapon Pelvis`, `WeaponSwordLeft`, `MOV`...) although `NODE_INVENTORY.md` measured 543 nodes in the winning `skeleton.nif`. Also: the DLL reports `pluginVersion 0.1.0.0` for the 0.2.0 build and still imports one modal API (redirected by the SKSE fork's automation shim). Post-revert preflight: the 3 pre-existing no-Keep blockers only (34705/50049/62001). Phase-2 acceptance still needs the crash reproduced or cleared, a character with a left-hand weapon, and the user's eyes on a sheathed left-hand weapon.
...
- **Source:** User linked it twice, then *"Let's go ahead and get the patches, but please vet them first."* **It is not only a fix mod** - it implements Morrowloot's rarity rules, removing daedric, glass, ebony, dragon, stalhrim, elven, orcish, dwarven and nordic gear from random leveled lists. Flagged before install. Survey 0/19, as is Morrowloot.
- **Parked immediately after install** at the user's instruction - *"Try not to activate new plugins right now, keep them inactive"* - because the parallel session is mid-bisect on a Sep 9 save crash. All three plugins are unstarred but hold their vetted load-order slots, ledger rows carry `disabledPlugins`, and activation is tracked in [#264](https://github.com/Ensrick/skyrim-mod-assistant/issues/264).
- **Verification:** **UNVERIFIED in game.** Vetted read-only first, with Astra holding the claim for a Sep 9 save-crash bisect: archives downloaded and inspected without touching the profile. Main is 1,239 records - 790 LVLI, 363 CONT, 4 Thieves Guild fence factions, 1 ARMA - on USSEP as a master, forwarding 99 of its records. Patches are 8 and 37 records with correct masters. **Placement was the whole risk:** at the tail it would have taken 26 records off `exchangeCurrency_enhanced`, 3 off `Ensrick Currency Integration Patch`, 5 off `exchangeCurrency_patch_COIN`, **22 of Simple Hunting Overhaul's 24 leveled lists**, 10 off `LostLongSwords` and 11 off Sons of Skyrim. After the move, all seven re-verified as owning their shared records. Resolver gate clean, `install_mod.py --verify` `0 problem(s)`, MO2 audit `errors: []`.

## 2026-09-08 21:26 — Regenerated the Pandora behavior graph (stale-graph behavior CTDs)

- **What:** Re-ran Pandora Behaviour Engine+ 4.4.0-beta (headless fork) through the MO2 VFS against the full current 304-plugin set, regenerating `Pandora Output - Ensrick` (Engine.log now 2026-09-08 21:26:45, was 2026-09-07 00:21; 186 HKX, OutputAnimData/OutputAnimSetData merged OK). Reverted the two wrong bisect edits first (restored `XPMSSE/scripts/XPMSEWeaponStyleScaleEffect.pex`, restored `SkyParkour v3 - SPPF/SKSE/Plugins/SkyParkourNG.dll`). SkyParkour was ruled out as a cause (crash-2026-09-08-20-49-22 occurred with its DLL disabled). No plugins added/removed by the regen (activeBefore=activeAfter=304, orderChanged=false).
- **Source:** Three behavior-graph CTDs #256 (crash-2026-09-07-20-57-35, -09-08-20-38-43, -09-08-20-49-22), all in the player behavior graph on XPMSE nodes (BodyWeightBlend/ArmsBlend/CarryRightArmBehavior), invalid pointers (0x1/-1). Root cause: the behavior graph was generated 2026-09-07 00:21 and a week of animation-affecting mod edits landed on top without a rerun (#128 rule). NOT a FNIS issue — Pandora converts XPMSE AA to OAR natively; the #148 fnis_aa Papyrus gap is longstanding (present in the 00:21 generation too) and did not correlate with crash onset. [[feedback-never-fall-back-to-deprecated-tools]].
- **Verification:** **UNVERIFIED in game — awaiting the user's play-test past the ~1 min gameplay window (harness cannot autoload a save: native-currency fresh-character gate).** Post-regen `install_mod.py --verify` 0 problems; `verify_order.py` CLEAN (304 active / 388 discoverable); Pandora Output enabled at modlist 121. Pre-regen Engine.log archived `records-work/pandora-engine-pre-regen-20260908.log`. Claim fable/pandora-regen. Pending on verified-stable: document + a preflight staleness gate (behavior graph must be newer than every animation/skeleton mod), per [[feedback-crash-fix-document-and-harden]] and #128.
...
- **What:** Four user-requested changes, one claimed session, one verification launch.
  (1) **Removed QuickLoot IE.** Disabled the MO2 mod `QuickLoot IE - Ensrick 1.7.99` (`mod-disable` `20260908T021916099Z-b5a21e8188f5`) and deactivated `QuickLootIE.esp` (`plugin-disable` `20260908T021953655Z-9afb92dc8eea`); the mod folder is kept for reversibility, not deleted. Its ledger row is now `enabled: false`. The QuickLoot family (QuickLoot IE, QuickLoot EE, LootMenu) is recorded **Skip** in [docs/KEEP_REVIEW.md](docs/KEEP_REVIEW.md); the installed mod was a local rebuild of the [Ensrick/QuickLootIE](https://github.com/Ensrick/QuickLootIE) fork (no Nexus id, so it does not trip the installed-Skip gate), and that fork plus PR #29 stay as-is, just shelved. (2) **TrueHUD floating enemy healthbars off:** `bEnableActorInfoBars = 0` in `mods/TrueHUD/MCM/Config/TrueHUD/settings.ini` (set to `0` by the team lead's inline edit at 21:11, backup `settings.ini.bak.v20260907-211138`; boss bars, player widget and recent-loot left `1`). (3) **Compass Navigation Overhaul location-name and distance popups suppressed:** `fAngleToShowMarkerDetails=0` and `fAngleToKeepMarkerDetailsShown=0` (vendor `10`/`35`, taken to `0.1` by the team lead's inline edit at 21:11 with backup `CompassNavigationOverhaul.ini.bak.v20260907-211138`, then to `0` per the original spec) in `mods/Compass Navigation Overhaul/SKSE/Plugins/CompassNavigationOverhaul.ini`, `fFocusingDelayToShow=600` left as-is, CRLF and encoding preserved. CNO has no MCM toggle for the focused-marker text, so this is best-effort trigger suppression, not a real off switch; if the text still shows, the fallback is removing CNO, which is the user's call. Enemy markers and the quest list were left alone. (4) Removing `QuickLootIE.esp` dropped the plug-in set 378 -> 377, so **Weapon Balance** was regenerated for the 376-input load order (MO2Vfs, `mod-stage --replace` `20260908T022541549Z-a8dfa9820ac3`; `WeaponBalancePatch.esp` bytes unchanged since QuickLoot has no WEAP records, 3504 WEAP overrides, deterministic double build, final-winner audit PASS) and **Full Cloak Exclusivity** rebuilt for 377 plug-ins (`mod-stage --replace` `20260908T022716205Z-ad4eab7bca80`; 240 directives byte-identical, 569/576 meshes present, 0 parse errors, 0 slot-58 hits). Before the batch, the MO2 GUI exit from the user's crashed session had re-drifted the master order a second time ([#253](https://github.com/Ensrick/skyrim-mod-assistant/issues/253), this time moving `AHZmoreHUD.esl`); `SmoothCam.esl` was restored to its vetted slot again (`plugin-priority`, final `20260908T020603260Z-3b3bae10c675`, backups `*.bak.v20260907-210546-pre-redrift-restore`) before any regeneration.
- **Source:** User: *"I don't really like quick loot, remove that, and skip the quick loot mods"*, *"turn off the floating healthbars"*, and *"turn off ... the names of locations and distances popping up on the compass."* The QuickLoot fork itself was the [#254](https://github.com/Ensrick/skyrim-mod-assistant/issues/254) gamepad-id crash fix. Receipts: [weapon](records/source-builds/ensrick-weapon-speed-balance.json), [cloak](records/source-builds/ensrick-full-cloak-exclusivity.json), [PopupGuard](records/source-builds/popup-guard-0.1.0.json).
- **Verification:** **MENU-ONLY (harness) 2026-09-07 21:30; HUD visuals and the crash remain UNVERIFIED pending the user's play session.** `launch_verify --no-autoload` reached `MAIN_MENU_OPEN` at ~40 s from process start (under 60 s), 52 plug-ins checked, 0 refused. `skse64.log` lists `checking plugin !PopupGuard.dll` first, `preloading plugin "PopupGuard"` before EngineFixes, `loaded correctly (handle 1)`, and **no `QuickLootIE` line** (0 mentions); `PopupGuard.log` shows all 10 hooks `ok`. No new crash log; the only `[critical]` is the known native-currency fresh-character gate (2 lines in `EnsrickCurrencyDenominations.log`, [#207](https://github.com/Ensrick/skyrim-mod-assistant/issues/207)/[#209](https://github.com/Ensrick/skyrim-mod-assistant/issues/209)/[#217](https://github.com/Ensrick/skyrim-mod-assistant/issues/217)). `install_mod.py --verify` 0 problems; weapon freshness PASS at 376 inputs; preflight clean (7 warnings). The #164 human guard left the game running (a Console menu opened 7 ms after the main menu, the recurring main-menu-init pattern; the user was at the machine), so it was not force-killed. The earlier in-session save-reload crash is tracked as [#256](https://github.com/Ensrick/skyrim-mod-assistant/issues/256) (a vanilla engine ButtonEvent/JobThread access violation, unrelated to this batch). Whether the floating healthbars and compass marker text are actually gone, and whether #256 recurs, need the user's next play session. Logs archived at [records/log-snapshots/20260907-213053-hud-removal/](records/log-snapshots/20260907-213053-hud-removal/).

## 2026-09-07 20:49 — PopupGuard installed; hidden dialogs become log lines; SmoothCam.esl slot restored

- **What:** Installed original PopupGuard 0.1.0 as the new mod `Ensrick - Popup Guard` (MO2Headless `mod-stage`, transaction `20260908T013056824Z-cb2479bb344f`, priority 299, next to Window Focus Guard 298). One SKSE DLL `!PopupGuard.dll` (sha256 `133DCD433D73F97121C6D6820CC787D6C4AEE172BF0E62024E28F7B79A660F64`), no plugin. MinHook 1.3.4 inline hooks on the exported user32 `MessageBoxA/W/ExA/ExW/TimeoutA/TimeoutW/IndirectA/IndirectW` and kernel32 `FatalAppExitA/W`: every call writes one escaped, bounded line (time, API, calling module plus offset, flags, caption, text) to `SKSEPopupGuard.log` and returns `IDOK` at once; `FatalAppExit` logs, then `TerminateProcess(1)`. Hooks install at DLL attach; the plugin exports `SKSEPlugin_Preload` and sorts first, so other plugins' load-time dialogs are covered. Opt-out `SKSE/Plugins/PopupGuard.ini` `Enabled=false`. Separately restored `SmoothCam.esl` to its vetted slot (managed active index 35, immediately after `TrueHUD.esl`, before `Thanedom Assets.esl`): the first preflight, before the install, had both generated-patch gates red because that one ESL had rotated two slots down when the 20:12 launch rewrote the profile files, the same order-only drift class as the two AHZmoreHUD restores earlier today (#253). Transactions `20260908T013622009Z-805ee9515efa` and `20260908T013735293Z-9724f6f0c087` (index-space corrections) and `20260908T013818488Z-54620e127e78` (final); backups `profiles/Default/loadorder.txt.bak.v20260907-203613-pre-smoothcam-restore` and `plugins.txt.bak.v20260907-203613-pre-smoothcam-restore`. Weapon Balance then passes freshness with no regeneration. Full Cloak Exclusivity was rebuilt for the new enabled-mod fingerprint and the corrected order (transaction `20260908T014631617Z-271b346caccb`): 240 directives byte-identical (INI `5716AE2F`), 569/576 meshes present, 0 parse errors, 0 slot-58 hits.
- **Source:** [#255](https://github.com/Ensrick/skyrim-mod-assistant/issues/255): during the 2026-09-07 19:01 hang ([#254](https://github.com/Ensrick/skyrim-mod-assistant/issues/254)) CommonLibSSE-NG `report_and_fail` parked the main thread in `MessageBoxW` behind the borderless 4K window, and 52 enabled SKSE DLLs import `MessageBox` or `FatalAppExit`, so per-fork patching does not scale. Source `mods/popup-guard` at `aeb8e4a` (CI run 34175043912 PASS); build and install receipt [records/source-builds/popup-guard-0.1.0.json](records/source-builds/popup-guard-0.1.0.json). Order-restore precedent: [docs/BTPS-CRASH-REPAIR-2026-09-07.md](docs/BTPS-CRASH-REPAIR-2026-09-07.md); drift tracked on [#253](https://github.com/Ensrick/skyrim-mod-assistant/issues/253).
- **Verification:** **VERIFIED by the user's own live session started 20:35:50 through the MO2 GUI, after the install and before the order restore (explicit user sign-off pending).** `skse64.log` lists `checking plugin !PopupGuard.dll` first, `preloading plugin "PopupGuard"` before EngineFixes and `plugin !PopupGuard.dll ... loaded correctly (handle 1)`, 52 plugins loaded, none refused; `PopupGuard.log` shows all 10 hooks `ok` at DLL attach, preload and load. LaunchProbe: main menu at 35.0 s, `kPostLoadGame success=1` at 50.8 s (human-loaded save), gameplay still running past 20:47 with no crash log and no `[critical]`; no dialog was raised in the session, so the intercept path is host-tested only (3 CTest groups, 100 checks, deterministic DLL across three rebuilds). Static: `audit/skse_version_data.py` PASS (explicit 1.7.104); `install_mod.py --verify` 0 problems; weapon freshness PASS; preflight clean with 7 documented warnings (its MO2 plugin and load-order checks were skipped because the session was live, #103). No assistant-driven launch: a human was at the controls the whole time (#164), so the exact final state after the 20:38 order restore and the 20:46 cloak refresh has not itself been launched. Process note: those two transactions ran while that session and its MO2 GUI were open because a Git Bash `tasklist /FI` check was mangled by MSYS path conversion; the running game had already read `plugins.txt` and the byte-identical cloak INI, the profile files were re-verified at 20:48, and the SmoothCam slot must be re-checked once the GUI closes. Logs archived at [records/log-snapshots/20260907-204800-popup-guard-255/](records/log-snapshots/20260907-204800-popup-guard-255/).

## 2026-09-07 20:13 — QuickLoot IE 1.7.104 gamepad-id freeze fixed and installed

- **What:** Rebuilt and DLL-only-replaced `QuickLoot IE - Ensrick 1.7.99` (MO2 priority 104) from the Ensrick fork branch `fix/1.7-gamepad-id` (commit `e49021c`, PR [Ensrick/QuickLootIE#29](https://github.com/Ensrick/QuickLootIE/pull/29)). `QUsingGamepad` now maps runtime `>= 1.7.0.0` to Address Library id `443396`; on 1.7.104 the previously selected `510926` (and `68622`) are absent from the version database, and the failed lookup when the loot menu first built its button bar drove CommonLibSSE `report_and_error` into a `MessageBoxW` hidden behind the borderless 4K window, then `TerminateProcess` - the apparent freeze. The CommonLib fatal path and QuickLoot's own `SanityChecks::ShowFatalError` were made non-modal (built against the Ensrick CommonLibSSE-NG `ensrick/no-modal-fail-v6.7.1` branch with `COMMONLIBSSE_NO_MODAL_ERRORS`); the resulting DLL imports neither `MessageBox` nor `FatalAppExit` and drops `user32.dll` from its import table. No plugin was added or removed and the other 19 payload files are byte-identical. Separately restored `AHZmoreHUD.esl` to its vetted load position (managed priority 37, immediately after `Thanedom Assets.esl`), which the user's crashed 19:01 session had drifted; this re-cleared the weapon-balance and full-cloak freshness gates with no artifact regeneration (the drift was order-only: identical 378 plugins, identical content).
- **Source:** [#254](https://github.com/Ensrick/skyrim-mod-assistant/issues/254). The disassembly evidence (`510926 -> 0`, `443396 -> 0xCF97B0` gamepad-enabled check), build, and install provenance are in [the source-build receipt](records/source-builds/ensrick-quickloot-ie-1799-1.7.104-gamepadfix.json). The load-order restore reapplies the vetted order documented in [the BTPS repair record](docs/BTPS-CRASH-REPAIR-2026-09-07.md) (drift tracked on [#253](https://github.com/Ensrick/skyrim-mod-assistant/issues/253)).
- **Verification:** **VERIFIED in game 2026-09-07 by the user's live play session (explicit user sign-off pending, per doctrine).** DLL sha256 `2298567D6705C9E8F3656BA3755EFF96879BA17A41DA8D8AFA276503B5C1BD52`; DLL-only replace transaction `20260908T005651616Z-4d9bd19be4bd`; order-restore transaction `20260908T010505226Z-be8e416efcfe`. `audit/skse_version_data.py` PASS (version independent); `dumpbin /imports` shows no `MessageBox`/`FatalAppExit`; `QuickLootIEPolicyTests` 18 assertions in 3 cases pass; `audit/install_mod.py --verify` 0 problems; preflight clean (6 warnings). `launch_verify.py --no-autoload --leave-running` reached the main menu at 43.0s with 52 SKSE plugins and 0 refused, and QuickLoot logged `Installed class QuickLoot::LootMenu as LootMenu` with 0 `[critical]`. The harness could not autoload a gameplay save (native-currency fresh-character gate), so a human at the controls (correctly left running per #164) then loaded a save and played: `QuickLootIE.log` records the exact prior-crash sequence `Blocking activation prompt 'Search Smuggler'` -> `Creating new loot menu instance` -> `Loading swf` eight times, with eight `MENU_OPEN name="LootMenu"` in `LaunchProbe.log`, zero `[critical]` and zero `Failed to find the id` across the whole session, no crash log, and a clean exit. Logs archived at [records/log-snapshots/20260907-201307-quickloot-254/](records/log-snapshots/20260907-201307-quickloot-254/).

## 2026-09-07 14:50 — SmoothCam zero-popup candidate 2 installed after candidate 1 failed

- **What:** Installed the untouched [SmoothCam](https://www.nexusmods.com/skyrimspecialedition/mods/41252) 1.7.1 AE/ESL package, preserving it as the base for `SmoothCam 1.7.104 Native Overlay - Ensrick` at MO2 priority 328. The vendor DLL failed the first manual 1.7.104 launch. Local candidate 1 then opened a native Address Library popup and stopped on ID `0`; this was traced to three unused zero-valued AE render placeholders. Candidate 2 no longer resolves those placeholders. SmoothCam warning, fatal, CommonLib fatal and minidump-notification paths were made non-modal while retaining logs and fail-fast behavior. The vendor ESL, MCM, scripts, presets, translations and assets are unchanged. Upstream has no license declaration at revision `66f3960`, so the build and symbols are local-only and must not be redistributed.
- **Source:** User required: *"I should be getting 0 popups or you've failed."* The earlier install directive also requires installed mods to be live Keeps. The 15-list survey found SmoothCam in 15/15 lists. Exact archive selection is in [the FOMOD plan](records/fomod-plans/41252-smoothcam-1.7.1-ae-esl.json), and full failure/build/install provenance is in [the receipt](records/smoothcam-1.7.104-install-2026-09-07.md).
- **Verification:** **FAILED for the vendor DLL and local candidate 1; candidate 2 UNVERIFIED in game.** All 46 Address Library IDs the AE build actually resolves are present with nonzero offsets in the installed 1.7.104 format-5 database. Candidate 2 hash `D50E9E489388C590D652C92D0C2ECB2BD363431DF7B284BD1AD3776D4212FCAD` matches the installed winner; its metadata declares plug-in version 18, Address Library V5 and exact runtime `1.7.104.0`. Its import table contains neither `MessageBox` nor `FatalAppExit`. Replacement transaction `20260907T195027816Z-660939414f37`. Nexus Keep 41252 remains **live**. The DLL-only replacement did not change generated fingerprints, but concurrent plug-in additions ending with Skyrim Reborn did; Weapon Balance was therefore regenerated from 377 inputs and Full Cloak Exclusivity over 378 runtime plug-ins. Both installed-freshness gates and the combined preflight pass. A fresh manual launch is still required before runtime acceptance.

## 2026-09-07 12:26 — Embers, CC fixes, functional UI and JS Shrines installed; Mihail utilities skipped

- **What:** Installed Embers XD 3.2.8 1K with the exact Community Shaders/Lux Orbis selection and four applicable patches; USCCCP 8.5 merged; TrueHUD 1.1.10; moreHUD 5.4.4 light-master; Infinity UI 2.0.3; Compass Navigation Overhaul 2.2.0; Better Third Person Selection 0.8.9; and JS Shrines of the Divines 1.3 2K. The outdated Bow of Shadows single-fix package is parked because USCCCP supersedes it. Infinity UI, Compass and BTPS retain their vendor packages and receive separate source-built Skyrim 1.7.104 DLL overlays. MihailMods Page Utilities (67811) is now **Skip**, scoped only to that utility page; the author's releases remain eligible for review and its useful article links are preserved in [the reference index](records/mihailmods-reference-index-2026-09-07.md).
- **Source:** User approvals: *"get Embers XD in there, along with the needed patches"*, *"USCCCP? Ok, add it"*, *"Let's add better third person selection"*, *"TrueHUD/moreHUD/Infinity UI/Compass Navigation Overhaul ... include them"*, and *"JS Shrines of the divines looks excellent"*. Exact selections, transaction IDs and native build provenance are in [the batch record](records/approved-ui-embers-uscccp-2026-09-07.md); adoption evidence is in [the follow-up survey](records/modlist-survey-2026-09-07-ask-list-followup.md).
- **Verification:** **UNVERIFIED in game; no game or MO2 GUI launched.** LOOT exit 0; 289 active / 373 discoverable plugins; install ledger, master/order, MO2 controller and record-conflict audits all pass. The three source DLL winners match their recorded build artifacts and pass the static 1.7.104 gate. Weapon balance was regenerated from 368 inputs and passes all 4,213 final-winner checks; full-cloak exclusivity was regenerated over 369 plugins with zero NIF parse failures or slot-58 hits. Keep coverage is clean at 219/219. Final preflight was clean apart from documented warnings; runtime behavior and visual acceptance remain outstanding.

## 2026-09-07 00:20 — Precision 2.0.6 installed and Pandora output regenerated

- **What:** Installed [Precision](https://www.nexusmods.com/skyrimspecialedition/mods/72347) 2.0.6 from exact MAIN file 798533 with the core payload and bundled `colis` behavior patch; no TK Dodge/Ultimate Combat compatibility files. Activated `colis` and regenerated the existing owned Pandora output through MO2's VFS: Precision is Pandora Mod 1, 186 HKX / 223 generated files, engine exit 0. The installed vendor configuration remains unedited, including trails, hitstop, recoil and their camera-shake defaults.
- **Source:** User: *"Precision I think I want. Let's add that."* Exact selection is recorded in [the FOMOD plan](records/fomod-plans/72347-precision-2.0.6.json). The associated current 15-list survey is [recorded here](records/modlist-survey-2026-09-07-ask-list-followup.md).
- **Verification:** **UNVERIFIED in game; no game or MO2 GUI launched.** Archive SHA-256 `9E5EE9957B012ED0CED0ADD274EE6477077DDDB10D68BD20A2ABB055E00D8E98`; `Precision.dll` identifies 2.0.6.0, V5 Address Library independence and passes the 1.7.104 static gate. The 248 selected files had zero loose-file collisions before installation. `Precision.esp` is enabled; LOOT exit 0; 281 active / 365 discoverable plugins; order audit clean; install ledger reports 0 problems. The dependent weapon patch was deterministically regenerated from 360 inputs; its final-winner gate caught and repaired a prior Wuuthrad ordering regression, after which all 4,212 target/preserve rows and installed freshness passed. The full-cloak slot-58 proof was regenerated for 361 runtime plugins: 240 directives, 569 present meshes, zero parse errors and zero partition-58 hits. Keep is queued and becomes final when the browser extension applies the seven-decision relay batch.
...
## 2026-09-07 23:05 — Man's Best Friend takes the Meeko's Shack slot

- **What:** Installed FuzzBeed's Asset Library (134092, v12.11), Man's Best Friend (160130, v2.0), its fix pack (167114) with the plain NOTWL patch, and the Skyrim Reborn disable patch (173788). Five plugins, moved ahead of the resolver block to plugins.txt 294-298.
- **Source:** User: *"I like this Meeko's shack."* Three mods wanted that location - Man's Best Friend, Meeko's Shack Reborn (179541) and Skyrim Reborn's own - and nothing measurable separates them, so the choice went to the user rather than being decided here. 173788 is what makes the pick work: it disables only Reborn's shack edits, keeping its other nine Hjaalmarch overhauls. The `NOTWL NC` fix variant was **not** taken; it targets Nature of the Wild Lands Nordic Cut, which this build's NotWL install explicitly excludes.
- **Verification:** **UNVERIFIED in game.** All five landed at plugins.txt 302-306, past the resolver block, and the resolver-order gate failed with 14 violations on `WRLD 00003C:skyrim.esm` - its second live catch in a day. Moved to 294-298, after Skyrim Reborn (292) and in dependency order; gate clean with its one standing advisory. The disable patch was verified to carry `Skyrim Reborn - Hjaalmarch Hold.esp` as a master, with 233 records (229 REFR). `install_mod.py --verify` `0 problem(s)`. Profile files backed up.
- **Note for the parallel session:** this batch adds five plugins on top of the 377-plugin count Sol regenerated Weapon Balance and Full Cloak Exclusivity against at 21:30, so both artifacts are stale again and need another pass before launch.

## 2026-09-07 22:43 — TrueHUD removed and skipped

- **What:** Removed TrueHUD (62775) entirely - plugin disabled, mod trashed recoverably, `TrueHUD.esl` pruned from `plugins.txt`/`loadorder.txt`, ledger row dropped, `skip 62775` queued through the keeps-pass route.
- **Source:** User, after a test launch: *"I hate HATE the unimmersive floating healthbars"*, then on being shown the full feature set - actor info bars, boss bars, player widget, recent loot, hide-vanilla-target-bar - *"the only thing I like is player bars repositioned bottom left, and I didn't even see that in-game, and it has a stupid widget symbol next to them... Remove true hud with prejudice and skip."* The floating bars were `bEnableActorInfoBars = 1` in `MCM/Config/TrueHUD/settings.ini`; moreHUD and Infinity UI were ruled out as the source (crosshair info and a framework respectively).
- **Verification:** Trash at `.mo2-headless-trash/20260908T034255865Z-a101bed6a9b3-TrueHUD`; profile files backed up as `.bak.v<stamp>-pre-truehud-removal`. `install_mod.py --verify` `0 problem(s)`, resolver-order gate clean. `keep_coverage` reports one expected transient - "Keep with nothing installed: 62775" - which clears when the queued skip is applied on the next Nexus page load. **Three mods call TrueHUD's API as a soft dependency and lose only its bar drawing: Better Third Person Selection, Open Animation Replacer, SkyParkour v3.** Better Third Person Selection is the one to eyeball next launch, since its selection bar was drawn through TrueHUD. No game process was running.

## 2026-09-07 15:25 — Skyrim Reborn - Hjaalmarch Hold, and the new gate earns its keep

- **What:** Installed Skyrim Reborn - Hjaalmarch Hold (23801, v1.1.4, mnikjom) with its Cutting Room Floor patch, since CRF is installed. Ten Hjaalmarch location overhauls in one plugin, 2,734 records. The Bards College Excavation optional was skipped - no such mod or CC file is present.
- **Source:** User: *"Well, I want Skyrim Reborn - Hjaalmarch Hold! It looks great!"* Note the author is mnikjom, not ThatDamnKapi, whose profile the user linked in the same message; Nexus profile pages return 403 to the assistant, so that author's catalogue needs an Ask Claude batch to enumerate.
- **Verification:** **UNVERIFIED in game.** Both plugins landed at plugins.txt 302/303, past the local resolver block, and **`audit/resolver_last_gate.py` failed with 16 violations**, including `WRLD 00003C:skyrim.esm` against Ensrick General Compatibility Patch - the first time the gate caught a live regression rather than a reconstructed one, roughly an hour after it was written. Moved to 293/294, ahead of the resolver block and still after CRF and USSEP; gate now clean with its one standing advisory. Asset overlap is 6 files against Snazzy Location Resources, won at priority 335. `install_mod.py --verify` `0 problem(s)`.
- **Open, and the user's call:** Skyrim Reborn includes its own Meeko's Shack overhaul, which competes with Man's Best Friend (160130) and Meeko's Shack Reborn (179541). If either standalone is chosen, patch 173788 disables only Reborn's shack edits and keeps its other nine locations.

## 2026-09-07 15:00 — Preflight gate: the resolver patches must load last

- **What:** New `audit/resolver_last_gate.py`, wired into `audit/preflight.py`. For every WRLD/CELL/LAND/NAVM record a resolver plugin overrides (`Ensrick Lux Water CS Patch.esp`, `Ensrick General Compatibility Patch.esp`), it fails if any plugin loading after it overrides the same record with different bytes. A byte-identical copy passes, which is what makes the Bloodchill forward legal.
- **Source:** User: "Yes", after the same failure landed three times in two days - Mesh Improvement Compilation, then PatchDirfjordFolkstead, each taking WRLD Tamriel off the General Compatibility Patch by sorting past it, and the Bloodchill forward avoiding it only because its WRLD record was deliberately authored as a byte copy. Skyrim replaces records whole, and any cell or worldspace patch must carry its parent WRLD, so this recurs on every install - a gate rather than vigilance.
- **Verification:** Gate reads the live profile in 0.12s and reports **clean** on the current order, with one advisory. Proven against the failure it was written for: a throwaway profile copy with `PatchDirfjordFolkstead.esp` moved back to last produces 2 blocking failures naming `WRLD 00003C:skyrim.esm` against both resolvers. Our own `Ensrick ...` patches are reported as advisory rather than blocking, since their relative order is deliberate - the standing one is the CRF Semantic Patch following the Lux Water patch on `CELL 006439:dawnguard.esm`. Preflight currently reports 6 unrelated blocking problems, none from this gate: a launch that died loading SmoothCam, a stale weapon-balance artifact, and four stale full-cloak reservation proofs, all downstream of today's installs and of Sol's SmoothCam work.

## 2026-09-07 14:40 — Thanedom of Dirfjord and its Folkstead patch

- **What:** Installed Thanedom Assets (47792, BSA version v1.3.2), Thanedom Of Dirfjord (85184, file v1.1) and A Thanedom Of Dirfjord - Folkstead Patch (189927, v1). Moved all three ahead of the local resolver block afterwards, to plugins.txt 291-293.
- **Source:** User linked the patch and asked "do we need it?" - the answer was no while its base mod was absent, then *"I like A Thanedom of Dirfjord though."* Dirfjord adds an Asen-clan town in the far south with a smith, general store, inn and one short quest; it requires Thanedom Assets and USSEP, so the assets pack came with it. Took the 1.5 MB BSA assets file, **not** the 43 MB "Thanedom Dialogue Assets", which the author describes as an addon for future Thanedom mods and which Dirfjord does not require.
- **Verification:** **UNVERIFIED in game.** The patch is exactly the border seam: 9 records - WRLD Tamriel plus `009CC3`, `009CC4` (DirfjordExterior01), `009CE4`, `009CE5` and their four LAND records - with 13 masters including Folkstead.esp. **Caught on install:** it landed at plugins.txt 301, past the resolver block, where its WRLD Tamriel record (MNAM `7e0a3019`) would have overridden Ensrick General Compatibility Patch's (MNAM `aa288a23`) - the same trap MIC hit two batches ago. All three Thanedom rows moved to 291-293, still after Folkstead (10) and in dependency order; the resolver block wins WRLD Tamriel again, re-verified. Profile files backed up. `install_mod.py --verify` `0 problem(s)`, MO2 audit `errors: []`.

## 2026-09-07 14:05 — JoopvanDie's border-town trilogy installed (ESMified)

- **What:** Installed Folkstead and the Border of Hammerfell (4018, v5.6), Haafstad and the Border of High Rock (4017, v9.8.1) and Nyhus and the Border of Cyrodiil (4016, v9), all as the **ESMified** variants so they are master-flagged and patchable. LOOT sort run afterwards, which placed them in the early block (plugins.txt 10, 17, 12).
- **Source:** User: *"get them all along with the necessary patches. Skip the alternate start patches, obviously."* **No patches were needed:** the ones those pages advertise target Rigmor of Bruma, Seapoint Settlement and Journey to Baan Malur, none installed; A Clear Map of Skyrim ships no border-town patch; and all three pages list BS Bruma and Beyond Reach - both installed - as compatible. The 2016 Alternate Start extensions were skipped as instructed and would not apply regardless, since this build runs Skyrim Unbound rather than Live Another Life. Survey: 0/19, expected for a 2016-origin trilogy outside the Wabbajack circuit.
- **Verification:** **UNVERIFIED in game, and these want a fresh character** - they add worldspaces, quests and NPCs. All three confirmed ESM-flagged (TES4 flag 0x1). Collision scan keyed by (master, local id) rather than raw formid, which is the only valid cross-plugin comparison: Folkstead 2,253 vanilla-space overrides, Haafstad 633, Nyhus 771, overwhelmingly their own border CELL and LAND. Between themselves 2-11 shared records, essentially the Tamriel WRLD header. Against the build: Landscape and Water Fixes 50 / 16 / 11, BS Bruma 6 / 2 / 2, Beyond Reach 3 / 2 / 2, Lux Via 4 / 2 / 5, Inigo 3 / 2 / 2. After the sort the local resolver block (Ensrick General Compatibility Patch, now last at 293) and `Ensrick Inigo Bloodchill Landscape Forward` (156) still win their records - re-verified. `install_mod.py --verify` `0 problem(s)`, MO2 audit `errors: []`.

## 2026-09-06 21:00 — Mesh Improvement Compilation added as the base layer under HPP

- **What:** Installed MIC (131131, v0.6.4) at priority 35, under High Poly Project (39), with the three of its 46 optional patches whose targets are in this build: Lux, Lux Orbis, Skyrim Landscape and Water Fixes. MIC - ENB Light skipped — the renderer here is Community Shaders (Lux CS installed, no d3d11/enbseries in the game folder) and MIC's page says to use CS Light instead.
- **Source:** User: *"MIC looks pretty good, why don't we include that too and put it before HPP, use it as more of a base layer."* Note MIC is **not** a lower-poly option — its own page warns that increased polygon counts "will inevitably impact your performance" and that it picks the high-poly variant where one exists. Survey: 2/19 exports carry it (Nordic Souls, CSVO), both *instead of* HPP.
- **Verification:** **UNVERIFIED in game.** Stack is SMIM (32) → SMIM Quality Addon (34) → MIC (35) + patches (36-38) → HPP (39) + Xtudo fixes (40-43) → Skyland (45) → Unofficial Material Fix (227) / Assorted Mesh Fixes (229). MIC wins 89 files against SMIM and 7 against the Quality Addon; loses 26 to HPP and the whole specialist layer above it. **Two problems found and fixed on install:** its ESP landed at plugins.txt 285, past the local resolver block, where its WRLD Tamriel record (differs only in MNAM, plus a 45,608-byte MHDT) overrode Ensrick General Compatibility Patch — moved to 268, ahead of that block; and it overrode HIMBO Refits on four male body meshes, which could not be fixed by raising HIMBO (The New Gentleman must keep winning its 27 files over it), so MIC's four copies were hidden with MO2's reversible `.mohidden` rename, as previously done for SkySight Skins. 1,304 asset paths, 440 MB, nothing over the 4096 ceiling. `install_mod.py --verify` `0 problem(s)`, MO2 audit `errors: []`.

## 2026-09-06 20:35 — High Poly Project installed with Xtudo's fixes, placed under the fix layer

- **What:** Installed High Poly Project (12029, v5.3) as the FOMOD's "Install Everything" — all 31 content folders, the Vaermina's Torpor plugin, and the **Campfire** firewood-snow patch (101 is the non-Campfire twin; 100 needs NSUTR, absent). Added Xtudo's fix set: AIO **Campfire** main (v2.0) plus the three update files that apply — Animated Chopped Wood v2.7 (2026-01-20, the current head), **Modern Lighting Overhaul** v2.5 (MLO2 is installed), and Hay Missing Wheat Textures v2.2. Four optional Xtudo patches skipped, targets absent.
- **Source:** User: *"Get the damn high poly project, just make sure we know why 9 seem to have chosen not to include it."* The why, from `docs/REQUESTED-MODS-REVIEW-2026-09-06.md:29-39`: Nordic Souls 3.0 and Lexy's July 2026 changelog both removed HPP on the record (Lexy: "no longer needed"); specific meshes are extreme — the [Garlic mod](https://www.nexusmods.com/skyrimspecialedition/mods/78848) measures HPP's garlic braid at **92,694 triangles / 5 draw calls against 5,016 / 2**, which is why LoreRim and Wunduniik run lower-poly coal overrides and Nolvus Ascension hides HPP's ingredients directory; it is unmaintained since 2021-10-22; and MIC (131131, v0.6.4) is now a maintained alternative. Correction to the earlier count: HPP ships no plugin, so the two plugin-only exports cannot show it — **10 of 17 measurable lists**, not 10/19.
- **Verification:** **UNVERIFIED in game.** Placement is the substance: HPP at priority **35**, above SMIM (32) and SMIM Quality Addon (34) so its geometry wins, below Skyland AIO 1K (41), Unofficial Material Fix (223), Assorted Mesh Fixes (225) and Skyrim Landscape and Water Fixes (232) so every targeted fix mod still wins — the "protected winners" rule that review asked for. Measured after the reorder: 272 asset paths; HPP wins 29 against SMIM and 8 firewood meshes against Traverse the Ulvenwald, and loses 22 / 14 / 3 / 1 to the fix layer, all intentional; Xtudo's four mods (36-39) win their 56 files over HPP. Textures: 106 DDS, 8 at 4096x4096, none above the ceiling; 562 MB. `install_mod.py --verify` `0 problem(s)`. `keep_coverage` shows 2 pending violations for 12029 and 63425 — their Keeps are queued in the relay spool and apply on the next Nexus page load.

## 2026-09-06 20:05 — Texture limits are ideals; SMIM Quality Addon un-shadowed; locks retiered to 2K

- **What:** Three changes. (1) `docs/TEXTURE_POLICY.md` rebalanced: the figures are ideals judged by screen coverage, 4096 stays the only hard ceiling. (2) **SMIM Quality Addon raised from priority 30 to 34, above SMIM (32)** — it had been shadowed since install, losing all 48 of its shared assets to SMIM: 43 reworked meshes and 5 higher-resolution textures (`smim_upperchest` .dds/_n/_weathered/_weathered_snow at 4096x4096 against 2048x2048, `smim_imp_gate_pole.dds` at 512x4096 against 256x2048). Unofficial Material Fix (218) and Assorted Mesh Fixes (220) still win their 6 and 2 files, as intended. (3) The four Security Overhaul lock mods re-installed at the **2K** tier.
- **Source:** User: *"treat the texture limits less as limits and more as ideal. The lockpicking interface is pretty large so having high res locksmithing is normal"* and *"You dropped something that was a downgrade from SMIM? You should definately look into that first."* The investigation corrected an earlier claim of mine: Some More Locks' two gate-pole textures are **byte-identical to SMIM's own** (sha256 `307a86dd7917` / `8b7ba56a2116`), so dropping them was a no-op at the time — but it is load-bearing now that the Quality Addon wins that path. `records/active-file-conflicts.md` had recorded the shadowing symptom without anyone acting on it.
- **Verification:** **UNVERIFIED in game.** After the change the Quality Addon wins its 48 shared files against SMIM and 1 against Traverse the Ulvenwald, and still loses 6 to Unofficial Material Fix and 2 to Assorted Mesh Fixes. Lock family now 83 DDS at 2048x2048 plus one 512x2048, nothing over the 4096 ceiling, ~421 MB against ~90 MB at 1K; asset-conflict scan still shows zero overlaps. `keep_coverage` clean, `install_mod.py --verify` `0 problem(s)`, MO2 audit `errors: []`. Priority transaction `20260907T010439764Z-db37694c8434`. Filed #251: `--replace` wiped the curated ledger `note`/`distribution` fields on the four retiered rows, which were rewritten by hand.

## 2026-09-06 16:20 — Security Overhaul lock family installed (5 mods)

- **What:** Installed Security Overhaul SKSE - Lock Variations (58224, v4.1.0) plus the four add-ons the ecosystem actually pairs with it: Lock Add-ons (59529), Regional Locks (62781), Extra Locks (126119) and Some More Locks (59961). All five take the 1K texture tier; Lock Variations takes the FOMOD's "v1.7.99+" runtime DLL. Nothing lock-related existed in the build before this.
- **Source:** User: *"Security Overhaul SKSE - Lock Variations looks great! ... Let's add it, and get the add-ons that suit our mods."* Survey of the 19 Load Order Library exports: Lock Variations **12/19** (the highest single-mod adoption measured in this survey), Regional Locks 10/19, Lock Add-ons 10/19, Some More Locks 8/19, Extra Locks 7/19, PBR 1/19. No competing lock-model mod exists in any list. Excluded on evidence: PBR (1/19, needs a confirmed PBR pipeline), BSAed (a repackage MO2 does not need), and the Ancient Dwemer Metal and Project AHO patches other lists carry, whose base mods are absent here.
- **Verification:** **UNVERIFIED in game.** Asset-conflict scan over every enabled mod: 123 asset paths across the five, **zero overlaps**, priorities 298-302 in dependency order. Textures: 83 of 87 DDS at 1024x1024 or below; two SMIM textures were dropped from Some More Locks because they would have downgraded SMIM Quality Addon's 512x4096 at the same path, and Lock Variations' 512x2048 `locksmallpiece_snow.dds` is flagged rather than capped (same 1,048,576-pixel budget as a compliant 1024 square, aspect preserved, byte-identical in all three vendor tiers). SKSE gate: `po3_LockVariations.dll` carries the `commonlib-shared` string, the fork migration that gives Address Library format 5 support on 1.7.104 — evidence, not proof; the launch log decides. `keep_coverage` clean at 206/206, `install_mod.py --verify` `0 problem(s)`, MO2 audit `errors: []`. FOMOD choices recorded in `records/fomod-plans/`.
