...
## 2026-09-07 13:49 — SmoothCam installed; stock DLL failed 1.7.104 and received a local source overlay

- **What:** Installed the untouched [SmoothCam](https://www.nexusmods.com/skyrimspecialedition/mods/41252) 1.7.1 AE/ESL package, then preserved it as the base for a separate `SmoothCam 1.7.104 Native Overlay - Ensrick` DLL/PDB mod at MO2 priority 328. The first manual launch stopped while SKSE was loading the released `SmoothCam.dll`; `skse64.log` ends on that plug-in and no SmoothCam log or crash log was created. Built an exact-runtime replacement from [upstream source](https://github.com/mwilsnd/SkyrimSE-SmoothCam) revision `66f3960` against CommonLibSSE-NG 6.7.1, migrating the small set of changed runtime-data interfaces. The vendor ESL, MCM, scripts, presets, translations and assets are unchanged. Upstream has no license declaration at that revision, so the build and symbols are local-only and must not be redistributed.
- **Source:** User: *"Let's install Smoothcam. Make sure we're adding things to keeps that are installed"*, followed after the failed launch by *"Can't due to smoothcam, I had some popup. We may need our own build for use with AI agents."* The 15-list survey found SmoothCam in 15/15 lists. Exact archive selection is in [the FOMOD plan](records/fomod-plans/41252-smoothcam-1.7.1-ae-esl.json), and build/install provenance is in [the receipt](records/smoothcam-1.7.104-install-2026-09-07.md).
- **Verification:** **FAILED for the released DLL; replacement UNVERIFIED in game.** Vendor failure is recorded from the user's manual attempt. The installed replacement hash `5173779445FDC9F3F6B6949210EA6C5CEA5EB9DE3FFB3A4B178DB4FF4D824388` matches the clean build artifact; its SKSE metadata declares plug-in version 18, Address Library format 5 and exact runtime `1.7.104.0`; imported DLLs are Windows/system components only. Install transaction `20260907T184934806Z-ac01b9a831fb`. Curator Keep 41252 is **live** in `decisions-applied-20260907-131045.json`; after the concurrent border-town batch, strict coverage is clean at 223/223. SmoothCam plus that completed batch required a deterministic refresh: Weapon Balance now covers 3,504 WEAP records from 372 inputs and passes 4,225 final-winner checks at the last managed position; Full Cloak Exclusivity passes over 373 runtime plug-ins with 569 parsed mesh winners and zero slot-58 hits. Final preflight is clean with five documented warnings and an exact-hash, one-test allowance for the replaced DLL. A fresh manual launch is still required before runtime acceptance.
