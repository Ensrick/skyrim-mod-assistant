...
| Skyland + full Nature of the Wild Lands foundation | User selected Skyland AIO 1K 4.32 as the broad base and full NotWL 3.14 as the tree overhaul. Both are installed and enabled with exact active-profile Bruma, Tundra Homestead, CRF, Lux Via, Grand Solitude, and Solitude Docks patches. Nordic Cut is rejected from the stack and is not installed; Mild Lands is not authorized. One 8K NotWL log map is capped by a deterministic local-only 4K overlay. Static MO2, order, master, loose-conflict, and semantic record audits pass. Runtime tree/frame-time/LOD routes remain on issue #29. Full record: `records/skyland-notwl-foundation-install-2026-08-30.md`. |
