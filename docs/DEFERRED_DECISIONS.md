...
| City/interior direction | Exact audit supports Grand Solitude 1.3.1 + Solitude Docks Updated 3.2c with their five-record official terrain patch. Snazzy Solitude AIO is only six non-Grand house interiors, while current SFCO3 BOS has exact Grand, Docks, and AIO/separated patches. Current Lux and Lux Orbis exact patches exist; Water for ENB, Nordic Cut/Docks, and final CELL/WRLD winners still require a small owned semantic ESP-FE and visual routes. eFPS Docks planes should not enter the first stability pass. | Choose Snazzy AIO versus separated houses and whether any JK city/Bards/Blue Palace/Outskirts module enters the stack; no installation has occurred. Full matrix: `records/solitude-city-interior-cell-matrix-2026-08-30.md`. |
...
| Tree overhaul + Morthal Bald Cypress | NotWL 3.14 remains the leading visual family. Its current main file already defaults to 2K bark diffuse/normal maps and 1K leaf diffuse/normal maps; the separate current texture-options archive chiefly offers 4K bark, 2K branches, or BC1-compressed 2K bark. Mild Lands is a stronger downscale: it halves every one of NotWL's 413 textures, yielding mostly 1K/512, 44 at 2K, and one 4K map. Nordic Cut is a placement profile that restores most normal trees to vanilla positions while keeping NotWL shrubs/debris and regional assets, reducing the placement-conflict surface. | User directed that Mild Lands remain unclassified—neither Keep nor Skip—until regular NotWL performance is measured. Decide full NotWL placement versus Nordic Cut; no tree package is installed yet. |
