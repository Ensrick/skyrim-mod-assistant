...
| SSEDisplayTweaks_Custom | `LockCursor` | **false, paired with WindowFocusGuard** | User's 2026-09-05 desktop-lock reproduction supersedes the earlier true setting. Official0.5.25 has an unconditional cursor-clip callback without a foreground check. The original WindowFocusGuard owns confinement instead, releasing on desktop intent/focus loss and refusing foreign windows. Do not run both cursor owners. Rendering/Havok settings remain unchanged; Media Keys Fix stays active. UNVERIFIED until real desktop/gameplay acceptance, #149. |
