...
internal static class LinkAudit
{
    ...
    {
        var listedModKeys = File.ReadAllLines(loadOrderFile)
            .Select(line => line.Trim())
            .Where(line => line.Length > 0 && !line.StartsWith('#'))
            .Select(line => ModKey.FromNameAndExtension(line.TrimStart('*')))
            .Where(modKey => modKey.FileName.String != Program.OutputPlugin)
            .ToArray();
        var modKeys = new[] { "Skyrim.esm", "Update.esm", "Dawnguard.esm", "HearthFires.esm", "Dragonborn.esm" }
            .Select(name => ModKey.FromNameAndExtension(name))
            .Concat(listedModKeys)
            .Distinct()
            .ToArray();
