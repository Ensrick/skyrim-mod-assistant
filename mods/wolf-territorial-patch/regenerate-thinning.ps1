...
<#
Regenerate "Ensrick Wolf Encounter Thinning.esp" (issue #42) from the live MO2 profile.
...
4. (the wolf record audit belongs to regenerate.ps1);
...
$pluginName = 'Ensrick Wolf Encounter Thinning.esp'
...
$work = Join-Path $PSScriptRoot 'thinning\work'
$package = Join-Path $PSScriptRoot 'thinning\package'
$spriggitText = Join-Path $PSScriptRoot 'thinning\spriggit'
$policy = Join-Path $PSScriptRoot 'thinning\policy.json'
...
# 4. (the wolf record audit belongs to regenerate.ps1; this half's receipts are the
#     cluster histogram the generator prints into work/generation-1.stdout.log)
...
foreach ($run in 1..2) {
    ...
    Invoke-Mo2Child -ChildPath $executable -ChildArguments @(
        ...
        '--ModKey', $pluginName,
        '--PatcherName', 'EnsrickWolfEncounterThinning',
        '--PersistencePath', (Join-Path $runFolder 'persistence'),
        '--ExtraDataFolder', (Join-Path $PSScriptRoot 'thinning')
...
$archive = Join-Path $package "Ensrick-Wolf-Encounter-Thinning-$Version.zip"
