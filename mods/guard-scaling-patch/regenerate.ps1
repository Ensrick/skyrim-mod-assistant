...
<#
Regenerate "Ensrick Guard Scaling Patch.esp" (issue #51) from the live MO2 profile.
...
4. runs the record audit through the MO2 VFS (work/guard-audit.json);
...
$auditJson = Join-Path $work 'guard-audit.json'
...
foreach ($run in 1..2) {
    ...
    Invoke-Mo2Child -ChildPath $executable -ChildArguments @(
        ...
        '--ModKey', $pluginName,
        '--PatcherName', 'EnsrickGuardScalingPatch',
...
$archive = Join-Path $package "Ensrick-Guard-Scaling-Patch-$Version.zip"
