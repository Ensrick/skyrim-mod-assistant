...
Bounded Encounters owns only actors that it creates. Ownership is recorded in a
runtime registry immediately when a generated actor is created. Authored actors,
actors made by other plugins, and references whose ownership is ambiguous are
never cleaned up or filtered by this plugin.
...
up through the narrowest engine-safe path.
...
The current alpha clears ownership and processed-cell state on a pre-load/new
session transition. It does not explicitly clean up live owned actors first.
Losing ownership before cleanup is a known gap: a candidate may proceed only on
disposable saves while tests determine whether the engine reliably reclaims
those temporary actors. A production design must either clean up resolved owned
handles safely before clearing state or prove that the transition always makes
them unreachable. Old handles and FormIDs must never leak across saves.
...
free to resolve according to Skyrim's own rules.
