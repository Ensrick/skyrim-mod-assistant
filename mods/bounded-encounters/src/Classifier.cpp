...
namespace BoundedEncounters
{
	...
	bool Classifier::Initialize()
	{
		...
		_bossRefType = RE::TESForm::LookupByEditorID<RE::BGSLocationRefType>("LocRefTypeBoss");
		...
		const bool a_allowDynamicReference) const
	{
		...
		const bool referenceRespawns =
			(a_actor->GetFormFlags() & RE::TESObjectREFR::RecordFlags::kRespawns) != 0;
		if (a_config.exclusions.nonRespawning &&
			(!actorBase->Respawns() || (!a_allowDynamicReference && !referenceRespawns))) {
