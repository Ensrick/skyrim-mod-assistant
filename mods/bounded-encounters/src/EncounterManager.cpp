...
namespace BoundedEncounters
{
	...
	bool EncounterManager::TrackSpawn(RE::Character& a_actor, const RE::FormID a_cellID)
	{
		...
		std::unique_lock lock(_stateLock);
		_spawnedHandles.insert(handle.native_handle());
	...
	void EncounterManager::ProcessCell(const RE::FormID a_cellID, const std::uint64_t a_session)
	{
		...
		const auto plan = BuildSpawnPlan(descriptors, _config.curves, playerLevel, cellSeed, effectiveCap);
		if (_config.observeOnly || _config.debugLogging) {
			for (const auto& candidate : candidates) {
		...
		for (const auto& roll : plan.sources) {
			...
			for (std::uint32_t index = 0; index < roll.extras && created < effectiveCap; ++index) {
				...
				if (!newActor) {
					if (newReference) {
						newReference->Disable();
						newReference->SetDelete(true);
					}
					++failures;
					continue;
				}
				...
				if ((newActor->GetFormFlags() & RE::TESForm::RecordFlags::kTemporary) == 0) {
					logger::error(
						...
						newActor->GetFormID());
					newActor->Disable();
					newActor->SetDelete(true);
					++failures;
					continue;
				}
				if (!newActor->MoveToNearestNavmesh(0.0F)) {
					logger::warn(
						"spawn rejected after navmesh placement failed: cell={:08X} source={:08X} spawned={:08X}",
						a_cellID,
						sourceActor->GetFormID(),
						newActor->GetFormID());
					newActor->Disable();
