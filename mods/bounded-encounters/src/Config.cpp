...
namespace BoundedEncounters
{
	namespace
	{
		...
		[[nodiscard]] nlohmann::json ParseWithoutDuplicateKeys(std::istream& a_stream)
		{
			std::unordered_map<int, std::unordered_set<std::string>> keysByDepth;
			const nlohmann::json::parser_callback_t rejectDuplicates =
				[&keysByDepth](
					const int a_depth,
					const nlohmann::json::parse_event_t a_event,
					nlohmann::json& a_parsed) {
					if (a_event == nlohmann::json::parse_event_t::object_start) {
						keysByDepth[a_depth + 1].clear();
					} else if (a_event == nlohmann::json::parse_event_t::key) {
						const auto& key = a_parsed.get_ref<const nlohmann::json::string_t&>();
						if (!keysByDepth[a_depth].insert(key).second) {
							throw std::runtime_error("configuration contains a duplicate JSON key: " + key);
						}
					} else if (a_event == nlohmann::json::parse_event_t::object_end) {
						keysByDepth.erase(a_depth + 1);
					}
					return true;
				};
			return nlohmann::json::parse(a_stream, rejectDuplicates, true, true);
		}
	}
	...
	Config LoadConfig(const std::filesystem::path& a_path)
	{
		...
		if (!stream) {
			throw std::runtime_error(
				"configuration file is missing from Data/SKSE/Plugins");
		}

		nlohmann::json root;
		root = ParseWithoutDuplicateKeys(stream);
		ValidateObjectShape(
			root,
		...
		ValidateObjectShape(
			limits,
			{ "maxAdditionalInterior", "maxAdditionalExterior", "maxHostilesInterior", "maxHostilesExterior", "maximumExteriorDistance", "placementRadiusMin", "placementRadiusMax", "maximumNavmeshSnapDistance" },
			"limits");
		...
		config.limits.maximumNavmeshSnapDistance = ReadFiniteFloat(
			limits, "maximumNavmeshSnapDistance", 0.0F, 4096.0F, "limits");
	...
	void ValidateConfig(const Config& a_config)
	{
		...
		if (a_config.limits.maximumExteriorDistance < 0.0F ||
			...
			a_config.limits.maximumNavmeshSnapDistance < 0.0F ||
			a_config.limits.maximumNavmeshSnapDistance > 4096.0F ||
			...
			!std::isfinite(a_config.limits.maximumNavmeshSnapDistance)) {
