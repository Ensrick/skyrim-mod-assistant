...
namespace
{
		...
		const std::uint32_t a_hardCellCap)
	{
		auto plan = BoundedEncounters::BuildSpawnPlan(
			a_sources, a_curves, a_playerLevel, a_seed, a_hardCellCap);
		// BuildSpawnPlan uses zero to mean "no global cap." Runtime creation uses
		// zero as a hard stop when cell or attached-area capacity is exhausted.
		// Normalize that API distinction so this audit reports actual creations.
		if (a_hardCellCap == 0) {
		...
		for (const auto category : kCategories) {
			...
			for (const auto level : kLevels) {
				...
				const auto plan = BoundedEncounters::BuildSpawnPlan(
					sources, config.curves, level, effectiveSeed, 0);
				rows.push_back({
					{ "level", level },
