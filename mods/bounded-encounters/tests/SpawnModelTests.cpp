...
namespace
{
	...
	[[nodiscard]] std::string CompleteConfigFixture()
	{
		return R"json({
			...
			"limits":{"maxAdditionalInterior":12,"maxAdditionalExterior":20,"maxHostilesInterior":24,"maxHostilesExterior":40,"maximumExteriorDistance":12000.0,"placementRadiusMin":96.0,"placementRadiusMax":256.0},
	...
	void TestExpectedValueMonotonicity(TestSuite& a_tests)
	{
		...
		for (std::uint32_t level = 0; level <= 100; ++level) {
			const auto plan = BuildSpawnPlan(sources, OneCurve(Category::General, curve), level, 0xA11CE, 0);
	...
	void TestDeterministicPlans(TestSuite& a_tests)
	{
		...
		const auto first = BuildSpawnPlan(sources, curves, 38, 0x123456789ABCDEF0ULL, 0);
		const auto second = BuildSpawnPlan(sources, curves, 38, 0x123456789ABCDEF0ULL, 0);
		...
		const auto anotherSeed = BuildSpawnPlan(sources, curves, 38, 0x0FEDCBA987654321ULL, 0);
	...
	void TestInputOrdering(TestSuite& a_tests)
	{
		...
		const auto forward = BuildSpawnPlan(ascending, curves, 44, 8675309, 0);
		const auto reverse = BuildSpawnPlan(descending, curves, 44, 8675309, 0);
		...
		std::ranges::sort(expectedAdmission, [](const SourceDescriptor& a_left, const SourceDescriptor& a_right) {
			const auto leftRank = MixSeed(8675309, a_left.sourceKey);
			const auto rightRank = MixSeed(8675309, a_right.sourceKey);
	...
	void TestCapsAndCategories(TestSuite& a_tests)
	{
		...
		const auto categoryCapped = BuildSpawnPlan(generalSources, curves, 10, 1, 0);
		...
		const auto mixed = BuildSpawnPlan(mixedSources, curves, 10, 9, 0);
		...
		const auto zeroMeansUnlimited = BuildSpawnPlan(
			generalSources,
			OneCurve(Category::General, unlimited),
			3,
			3,
			0);
		a_tests.Check(zeroMeansUnlimited.totalExtras == 12, "zero source, category, and global caps mean unlimited");
	}
	...
	void TestExcludedAndIneligibleSources(TestSuite& a_tests)
	{
		...
		const auto plan = BuildSpawnPlan(sources, curves, 10, 99, 0);
