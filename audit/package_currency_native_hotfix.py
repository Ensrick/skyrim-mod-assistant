...
def main():
    ...
    payload['SOURCE.txt'] = f'''Ensrick Currency Native Initialization Fix 0.2.2
...
'''.encode()
    payload['NOTICE.txt'] = b'''Ensrick Currency Native Initialization Fix 0.2.2
...
The old native DLL has known initialization defects and is not a working
currency fallback; rollback therefore restores the prior broken state.
'''
    payload['Source/Packaging/package_currency_native_hotfix.py'] = Path(__file__).read_bytes()
    args.output.mkdir(parents=True, exist_ok=True)
    output = args.output / 'Ensrick-Currency-Native-Initialization-Fix-0.2.2.zip'
    ...
    result.update(status='packaged-hotfix-not-installed', nativeVersion='0.2.2',
