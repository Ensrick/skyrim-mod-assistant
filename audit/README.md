...
| `verification_status.py` | proves linked plans still match the current build and rejects missing evidence, incomplete stages, false final status, failed/aborted runs, and post-policy installs with no plan (#227) |
| `verification_result.py` | ingests ledger/contract/fingerprint-bound result receipts; automated passes require an external hashed diagnostic, and `--human-acceptance` is the separate explicit V7 capability |
...
The game-folder baseline is schema 2: normalized paths relative to the Skyrim
root plus an exact SHA-256 for every tracked runtime file. Size and nanosecond
mtime are diagnostic fields, not identity. A legacy basename/size/mtime
baseline blocks ordinary preflight until it is inspected and migrated with the
issue-bound `preflight.py --accept-game-folder-baseline ISSUE` command.

### Work claim: `claim.py`

```
$env:SKYRIM_CLAIM_OWNER = '<you>'
$env:SKYRIM_CLAIM_LEASE = [guid]::NewGuid().ToString('N')
py -3 audit/claim.py acquire --owner <you> --purpose "multi-command work" --ttl 30
...
it. A pid-bound claim remains live while its exact owning process is alive even
after the advisory TTL; an unbound/manual claim past its TTL is stale. Nested
guards require the exact lease ID or creating PID: a shared friendly owner
name never makes two sibling agents the same holder. Every stale takeover is
logged (`records/claim-log.jsonl`). `install_mod.py` additionally refuses to mutate the
