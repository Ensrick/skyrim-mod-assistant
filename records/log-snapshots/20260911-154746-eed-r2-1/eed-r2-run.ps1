...
function WaitFor([string]$file, [string]$pattern, [int]$seconds, [string]$what) {
  ...
  while ((Get-Date) -lt $deadline) {
    if (Test-Path -LiteralPath $file) {
      $hit = Select-String -LiteralPath $file -Pattern $pattern -SimpleMatch | Select-Object -Last 1
      if ($hit) { Say "$what : $($hit.Line)"; return $hit.Line }
    }
    Start-Sleep -Seconds 2
  }
  Fail "timed out after $seconds s waiting for '$pattern' in $file ($what)"
}
...
'launch' {
  ...
  $launch = Start-Process -FilePath 'pwsh' -PassThru -WindowStyle Hidden `
    -ArgumentList @('-NoProfile', '-File', $launcher, '-WaitSeconds', $WaitSeconds, '-TestProfileName', ('"' + $profileName + '"'),
                    '-SourceProfileName', 'Default', '-ClaimOwner', $ClaimOwner) `
