# EED R1 verification driver: fresh character through Skyrim Unbound, via MenuPilot.
# Adapted from skyrim-mod-assistant/records-work/ussep439c-fresh-ui-20260910.ps1
# (Astra) and the verified quit recipe in docs/MENUPILOT.md. One action per call so
# every step is observable and the caller decides the next one from the readback.
param(
  [Parameter(Mandatory)][int]$OwnedPid,
  [ValidateSet('Main','MainKey','New','Race','MCM','Pages','Page','Keys','Open','Options','Nav','Accept','Read','CityDialog','Begin','Ping','Quit','Dump')][string]$Action,
  [string]$Key='Down',
  [int]$Count=1,
  [string]$Expect='',
  [string]$Path='',
...
$claim=& $py "$root\audit\claim.py" check --owner fable/eed-r1
...
$codes=@{Accept=28;Down=208;Up=200;Left=203;Right=205;Journal=36;Cancel=1}
...
switch($Action){
 ...
 'MainKey'{ Step ((@((Tap $Key))*$Count)+@((ReadField 'Main Menu' "$main.MainList.selectedEntry.text"))) $Expect }
 'New'{
 ...
 'Page'{
  # Focus the page list, move to the wanted page, open it.
 ...
 'CityDialog'{
  ...
  Step ((@((Tap 'Down'))*3)+@((ReadField 'Journal Menu' "$config._optionsList._selectedIndex"))) '6'
  Step @((Tap 'Accept'),$wait,(Cmd 'gfx.dump' @{menu='Journal Menu';path="$config.dialog.menuList._entryList";depth=2;max=130}))
 }
 'Begin'{
  Step @((Tap 'Left'),(ReadField 'Journal Menu' "$config._subList._selectedIndex"))
  Step ((@((Tap 'Up'))*$Count)+@((Tap 'Accept'),$wait,(ReadField 'Journal Menu' "$config._optionsList._entryList.0.text"))) 'Begin Your Adventure'
  Step @((Tap 'Right'),(ReadField 'Journal Menu' "$config._optionsList._selectedIndex")) '0'
  Step @((Tap 'Accept'))
 }
